...
def view_user(username):
    ...
    for r in reviews_col.find({"user_id": user['_id']}):
        ...
        review = {
            ...
            "createdAt": r.get("createdAt").strftime("%Y-%m-%d") if r.get("createdAt") else "",
...
def reviewpage(movie_id):
    ...
    reviews_cursor = reviews_col.find({"tmdb_id": movie_id})
    reviews = []
    ...
    for r in reviews_cursor:
        ...
        reviews.append({
            ...
            "userRating": r.get("user_rating"),
            "content": r.get("reviewText", ""),
            "username": review_author,
            "date": r.get("createdAt").strftime("%Y-%m-%d") if r.get("createdAt") else "",
            "likes": r.get("likes", 0),
...
def submit_review():
    ...
    if not content or not review_title or not movie_id or user_rating is None:
        return jsonify({"error": "Missing required fields"}), 400

    # Classify sentiment
    sentiment_label = classify_sentiment(content)
...
def get_reviews_for_movie(movie_id):
    reviews_list = list(reviews_col.find({"movieId": movie_id}))
...
def userpage():
    ...
    for r in reviews_col.find({"user_id": user['_id']}):
        ...
        review = {
            ...
            "reviewTitle": r.get("reviewTitle", ""),
            "reviewText": r.get("reviewText", ""),
            "release_date": release_date[:4] if release_date else "Unknown",
            "user_rating": r.get("user_rating", 0),
            "createdAt": created.strftime("%Y-%m-%d") if created else "Unknown"
        }
