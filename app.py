...
db = client.test
users_col = db.user
reviews_col = db.review
watchlists_col = db.watchlist
...
def userpage():
    ...
    user_watchlists = list(watchlists_col.find({"user_id": user['_id']}))
...
def update_review(review_id):
    data = request.json
    update_result = reviews_col.update_one(
        {"_id": ObjectId(review_id)},
        {"$set": {
            "review_title": data.get("review_title"),
            "user_rating": float(data.get("user_rating")),
...
def get_movie_release_date_by_id(tmdb_id):
    tmdb_api_key = TMDB_API_KEY  
    url = f"https://api.themoviedb.org/3/movie/{tmdb_id}"
    params = {
        "api_key": tmdb_api_key,
    ...
    try:
        ...
        print(f"[TMDB ERROR] Failed to get movie release date: {e}")
        return "Unknown Release Date"

@app.route('/watchlist/<watchlist_id>', methods=['GET'])
def get_watchlist_item(watchlist_id):
    try:
        # Convert string ID to ObjectId
        watchlist_obj_id = ObjectId(watchlist_id)
    except InvalidId:
        return jsonify({"error": "Invalid watchlist ID format"}), 400

    # Find watchlist entry by _id
    watchlist_item = watchlists_col.find_one({"_id": watchlist_obj_id})

    if not watchlist_item:
        return jsonify({"error": "Watchlist item not found"}), 404

    # Convert ObjectId fields to string for JSON serialization
    watchlist_item['_id'] = str(watchlist_item['_id'])
    watchlist_item['user_id'] = str(watchlist_item['user_id'])

    return jsonify(watchlist_item)
...
def add_to_watchlist():
    ...
    data = request.json
    movie_id = data.get('tmdb_id')
    title = data.get('title')
    average_rating = data.get('average_rating')
    release_date = data.get('release_date', get_movie_release_date_by_id(movie_id))
    ...
    existing = watchlists_col.find_one({"user_id": user_id_str, "tmdb_id": movie_id})
    ...
    watchlists_col.insert_one({
        ...
        "tmdb_id": movie_id,
        "title": title,
        "average_rating": average_rating,  # pass from frontend if available
